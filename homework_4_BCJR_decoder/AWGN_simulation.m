...
N_bit= 2000;
rate =0.5;
snr_db = 0:4;   % SNR := E_b/N_0 (E_b =1), noise ~ N(0, N_0/2)=N(0,1/(2*SNR)) 
snr = 10.^(snr_db/10);
N_C = [100,100,50,100,100]*2;
% N_C =[20,20,20,30,30];
...
%         y_4 =2*code_4 -1;
        %------------------ awgn channel------------------------------------
        recv_1 = y_1 + randn(1, length(code_1))/sqrt(snr(i));
        recv_2 = y_2 + randn(1, length(code_2))/sqrt(snr(i));
        recv_3 = y_3 + randn(1, length(code_3))/sqrt(snr(i));
...
%         ap_LLR_4 = zeros(1,rate*length(code_4));
        [~, ~, ~, dec_1] = mogen_log_map_bcjr(recv_1, trellis_1, rate*snr(i), ap_LLR_1,'term');
        [~, ~, ~, dec_2] = mogen_log_map_bcjr(recv_2, trellis_2, rate*snr(i), ap_LLR_2,'term');
        [~, ~, ~, dec_3] = mogen_log_map_bcjr(recv_3, trellis_3, rate*snr(i), ap_LLR_3,'term');
...
legend('NR-(5,7) code','NR-(27,31) code','SR-(7,5) code');
